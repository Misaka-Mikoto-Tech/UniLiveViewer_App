using UnityEngine;
//using UnityEngine.Animations.Rigging;
using VRM;

namespace UniLiveViewer 
{
    public class LookAt_VRMBone : LookAtBase, IHeadLookAt, IEyeLookAt, ILookAtVRM
    {
        [Header("＜LookAt(VRM用、自動)＞")]
        public VRMLookAtBoneApplyer_Custom _vrmEyeApplyer;

        protected override void Awake()
        {
            base.Awake();

            // TODO: 実行順見直す
            //チェック
            //if (_charaCon.charaInfoData.charaType != CharaInfoData.CHARATYPE.VRM_Bone)
            //{
            //    Debug.LogError("LookAtタイプが異なります");
            //    this.enabled = false;
            //}
        }

        /// <summary>
        /// 頭の注視処理
        /// </summary>
        void IHeadLookAt.HeadUpdate()
        {
            HeadUpdateBase();
        }

        void IHeadLookAt.HeadUpdate_OnAnimatorIK()
        {
            //全体、体、頭、目
            _animator.SetLookAtWeight(1.0f, 0.0f, _leapVal_Head, 0.0f);
            _animator.SetLookAtPosition(test.lookTarget_limit.position);
        }

        /// <summary>
        /// 目の注視処理
        /// </summary>
        void IEyeLookAt.EyeUpdate()
        {
            EyeUpdateBase();

            _result_EyeLook.x = 90;
            _result_EyeLook.y = _leapVal_Eye * 90;

            //目にオフセットを反映
            _vrmEyeApplyer.HorizontalOuter.CurveXRangeDegree = _result_EyeLook.x;
            _vrmEyeApplyer.HorizontalOuter.CurveYRangeDegree = _result_EyeLook.y;
            _vrmEyeApplyer.HorizontalInner.CurveXRangeDegree = _result_EyeLook.x;
            _vrmEyeApplyer.HorizontalInner.CurveYRangeDegree = _result_EyeLook.y;

            _vrmEyeApplyer.VerticalDown.CurveXRangeDegree = _result_EyeLook.x;
            _vrmEyeApplyer.VerticalDown.CurveYRangeDegree = _result_EyeLook.y;
            _vrmEyeApplyer.VerticalUp.CurveXRangeDegree = _result_EyeLook.x;
            _vrmEyeApplyer.VerticalUp.CurveYRangeDegree = _result_EyeLook.y;
        }

        /// <summary>
        /// 注視の有効/無効
        /// </summary>
        void ILookAtVRM.SetEnable(bool isEnable)
        {
            _vrmEyeApplyer.enabled = isEnable;
        }

        /// <summary>
        /// 注視リセット
        /// </summary>
        void ILookAtVRM.EyeReset()
        {
            _vrmEyeApplyer.HorizontalOuter.CurveXRangeDegree = 0;
            _vrmEyeApplyer.HorizontalOuter.CurveYRangeDegree = 0;
            _vrmEyeApplyer.HorizontalInner.CurveXRangeDegree = 0;
            _vrmEyeApplyer.HorizontalInner.CurveYRangeDegree = 0;

            _vrmEyeApplyer.VerticalDown.CurveXRangeDegree = 0;
            _vrmEyeApplyer.VerticalDown.CurveYRangeDegree = 0;
            _vrmEyeApplyer.VerticalUp.CurveXRangeDegree = 0;
            _vrmEyeApplyer.VerticalUp.CurveYRangeDegree = 0;
        }

        /// <summary>
        /// 注視ターゲット
        /// </summary>
        Transform ILookAtVRM.GetLookAtTarget()
        {
            return _lookTarget;
        }
    }
}