    ...
    public class ComponentAttacher_VRM : MonoBehaviour
    {
        ...
        public async UniTask Attachment(VRMTouchColliders touchCollider, CancellationToken token)
        {
            try
            {
                ...
                {
                    await _matConverter.Conversion_Item(meshRenderers.ToArray(), token);
                }
                ...
                //skinの方はVRMから流用
                await _matConverter.Conversion(CharaCon, token);
                await _matManager.ExtractMaterials(CharaCon, token);
        ...
        async UniTask CustomizeComponent_VRM(VRMTouchColliders touchCollider, CancellationToken token)
        {
            try
            {
                ...
                Destroy(_targetVRM.GetComponent<VRMFirstPerson_Custom>());//今後使うかも
